...
def interpLamFlame(cbDict,solFln):
    ...
    # if zipfile.is_zipfile('canteraData.zip'):
    #     fz=zipfile.ZipFile('canteraData.zip','r')
    #     for file in fz.namelist():
    #         fz.extract(file)
    #     os.chdir('./canteraData')
    # else:
    #     print("canteraData.zip does NOT exit!")
    #     return 300
    ...
    with open('lamParameters.txt') as f:   
        ll = 0
        for line in f:
            if ll == 0:
                casename = line.strip()     
    ...
    streamBC = np.loadtxt('lamParameters.txt',
                          skiprows=1+cbDict['nchemfile'])    
                          #将边界条件数据BCdata读取到streamBC中

    with open('./' + cbDict['output_fln'] ,'w') as strfile:             
    ...
    for i in range(cbDict['nchemfile']):    #对于每个当量比phi：
        fln = (casename + '_' + str('{:03d}'.format(i)) + '.csv')
    ...
    d2Yeq_table = []
    if(cbDict['scaled_PV']):
        d2Yeq_table = generateTable2(lamArr[:,1],Yc_eq,cbDict["z"],cbDict["gz"])     
      ...
      #输入：lamArr[:,1] 混合物分数； Yc_eq 反应速率； contVarDict 初始的z、c、gz、gc、gcz(都是向量)
        ...
        print('\nWriting --> ' + fln)
        np.savetxt(fln,d2Yeq_table,fmt='%.6e')   
    ...
    for i in range(cbDict['nchemfile']):   
      #对于每个当量比i：
        ...
        fln = ('Unf_'+casename+'_'+str('{:03d}'.format(i))+'.dat')  
    ...
    fln = 'chemTab_' + str('{:02d}'.format(cbDict['solIdx'])) + '.dat'   
...
def interp2D(M_Zc,Z_pts,c_pts,z_space,c_space):   
    ...
    f = scipy.interpolate.interp2d(c_pts,Z_pts,M_Zc)
    intpM_Zc = f(c_space,z_space)
    return intpM_Zc

def generateTable2(lamArr,Yc_eq,z,gz):     
